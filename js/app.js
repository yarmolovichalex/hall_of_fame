...
var addAlbumOnPage = function(item)
	{
		...
		var $band = $("<p>").text("Band: " + item.Band).appendTo($album);
		var $name = $("<p>").text("Album name: " + item.Name).appendTo($album);
		var $year = $("<p>").text("Year: " + item.Year).appendTo($album);

		var $imageLink = $("<a>").addClass("fancyBox").attr("rel", "group").attr("href", item.Image).appendTo($album); 
		var $image = $("<img>").attr("src", item.Image).appendTo($imageLink);

		var $showDetails = $("<button>").text("Подробнее").addClass("showDetails")
